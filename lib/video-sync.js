...
export async function syncVideos({ cfg, client, clientId, advertiserIds = [], months, fetchImpl = fetch, detailTopN = 10, detailBudgetMs = Infinity, now = () => Date.now(), sleep, fetchCover = fetchOembedThumbnail, coverTopN = 40, coverBudgetMs = 30_000 }) {
  ...
  for (const month of months) {
    ...
    try {
      ...
      // Engagement for the top-N clips by views that we have NEVER measured. TikTok caps this
      // endpoint with a rate limit shared across the whole app (429, code 36009002), and
      // keepEngagement below stores what we measure for good — so re-measuring a clip buys
      // nothing and spends quota every other client of the app also needs. Measure once, ever.
      // The cost of that: for the month still in progress a clip's likes/comments stay at the
      // value first recorded. The figures the dashboard leads with — views, GMV, orders — come
      // from the list endpoint instead and stay live, which is why this is the right thing to
      // freeze. Each call is still isolated: a failure only means "not measured yet" for that clip.
      const top = [...rows]
        .sort((a, b) => (b.views || 0) - (a.views || 0))
        .filter((r) => !measured.has(String(r.video_id)))
        .slice(0, detailTopN);
      const detailById = {};
